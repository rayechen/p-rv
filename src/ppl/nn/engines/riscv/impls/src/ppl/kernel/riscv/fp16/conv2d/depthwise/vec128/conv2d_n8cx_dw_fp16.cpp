...
void ppl3_conv_dw_src_padding_fp16(
    const __fp16 *src,
    __fp16 *src_padded,
    int src_h,
    int src_w,
    ...
    int src_padded_h_stride = src_w_padded * 8;
...
size_t ppl3_conv_dw_get_cvt_flt_size_fp16(
    int flt_h,
    int flt_w,
    int channels) {

    int padded_channels = (channels + 8 - 1) / 8 * 8;
    return size_t(flt_h * flt_w * padded_channels) * sizeof(__fp16);
}

void ppl3_conv_dw_cvt_flt_fp16(
    const __fp16 *flt,
    __fp16 *cvt_flt,
    int flt_h,
    int flt_w,
...
size_t ppl3_conv_dw_get_temp_buffer_size_fp16(
    int src_h,
    int src_w,
    int pad_h,
    int pad_w,
    ...
    int padded_channels = round_up(channels, 8);
...
void ppl3_conv_dw_kernel_riscv_fp16(
    const __fp16 *src,
    const __fp16 *flt,
    const __fp16 *bias,
    __fp16 *dst,
    ...
    int dst_w_div8 = dst_w >> 3;
    int dst_w_left = dst_w - (dst_w_div8 << 3);

    for (int i = 0; i < dst_h; i++) {
        for (int j = 0; j < dst_w_div8; j++) {
            int dst_idx = 0;
            dst_idx += i * dst_w * 8;
            for (int k = 0; k < 8; k++) {
                dst_idx += k;
            ...
            for (int hk = 0; hk < flt_h; hk++) {
                for (int wk = 0; wk < flt_w; wk++) {
                    for (int k = 0; k < 8; k++) {
                        ...
                        dst_idx += k;
                        int src_idx0 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 0) * stride_w + wk) * 8 + k;
                        int src_idx1 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 1) * stride_w + wk) * 8 + k;
                        int src_idx2 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 2) * stride_w + wk) * 8 + k;
                        int src_idx3 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 3) * stride_w + wk) * 8 + k;
                        int src_idx4 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 4) * stride_w + wk) * 8 + k;
                        int src_idx5 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 5) * stride_w + wk) * 8 + k;
                        int src_idx6 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 6) * stride_w + wk) * 8 + k;
                        int src_idx7 = (i * stride_h + hk) * src_pad_w * 8 + ((j * 8 + 7) * stride_w + wk) * 8 + k;
        ...
        for (int j = 0; j < dst_w_left; j++) {
            ...
            for (int hk = 0; hk < flt_h; hk++) {
                for (int wk = 0; wk < flt_w; wk++) {
                    for (int k = 0; k < 8; k++) {
                        int flt_idx = k + wk * 8 + hk * flt_w * 8;
                        int src_idx = (i * stride_h + hk) * src_pad_w * 8 + ((dst_w_div8 + j) * stride_w + wk) * 8 + k;
                        int dst_idx = k + (dst_w_div8 * 8 + j) * 8 + i *dst_w * 8;
...
uint64_t conv2d_n8cx_dw_fp16_runtime_executor::cal_temp_buffer_size() {
    size_t temp_buffer_size = ppl3_conv_dw_get_temp_buffer_size_fp16(
        src_shape_->GetDim(2),
        src_shape_->GetDim(3),
        conv_param_->pad_h,
        conv_param_->pad_w,
...
ppl::common::RetCode conv2d_n8cx_dw_fp16_runtime_executor::execute() {
    ...
    typedef void (*depthwise_riscv_kernel_fp16) (
        const __fp16*, const __fp16*, const __fp16*, __fp16*, int32_t, int32_t, int32_t
    );
    ...
    if (kernel_h == 3 && kernel_w == 3 && stride_h == 1 && stride_w == 1) {
        ...
        {
        case 0:{
            dw_conv_kernel = ppl3_conv_dw_f3s1_h1w4_kernel_riscv_fp16<0>;
            break;
        }
        case 1:{
            dw_conv_kernel = ppl3_conv_dw_f3s1_h1w4_kernel_riscv_fp16<1>;
            break;
        }
        case 2:{
            dw_conv_kernel = ppl3_conv_dw_f3s1_h1w4_kernel_riscv_fp16<2>;
            break;
        }
        case 3:{
            dw_conv_kernel = ppl3_conv_dw_f3s1_h1w4_kernel_riscv_fp16<3>;
            break;
        }
        default:
            break;
        ...
        case 0:{
            ...
            {
            case 0:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<0, 0>;
                break;
            }
            case 1:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<0, 1>;
                break;
            }                
            case 2:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<0, 2>;
                break;
            }
            case 3:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<0, 3>;
                break;
            }
            default:
                break;                
        ...
        case 1:{
            ...
            {
            case 0:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<1, 0>;
                break;
            }
            case 1:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<1, 1>;
                break;
            }                
            case 2:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<1, 2>;
                break;
            }
            case 3:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<1, 3>;
                break;
            }
            default:
                break;                
        ...
        case 2:{
            ...
            {
            case 0:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<2, 0>;
                break;
            }
            case 1:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<2, 1>;
                break;
            }                
            case 2:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<2, 2>;
                break;
            }
            case 3:{
                dw_conv_kernel = ppl3_conv_dw_f3s2_h3w4_kernel_riscv_fp16<2, 3>;
                break;
            }
            default:
                break;                
    ...
    for (int i = 0; i < padded_channels; i += 8) {
        ppl3_conv_dw_src_padding_fp16(
            src_ + i * src_h * src_w,
        ...
        if ((kernel_h == 3 && kernel_w == 3 && stride_h == 1 && stride_w == 1) ||
            ...
            ppl3_conv_dw_kernel_riscv_fp16(
                (__fp16*)temp_buffer_ + i * src_h_padded * src_w_padded,
                cvt_filter_ + i * kernel_h * kernel_w,
                cvt_bias_ + i,
                dst_ + i * dst_h * dst_w,
...
ppl::common::RetCode conv2d_n8cx_dw_fp16_offline_manager::pick_best_tunning_param(const __fp16 *src, const __fp16 *filter, __fp16 *dst,
...
ppl::common::RetCode conv2d_n8cx_dw_fp16_offline_manager::gen_cvt_weights(const __fp16 *filter, const __fp16 *bias) {
    ...
    {
        cvt_filter_size_ = ppl3_conv_dw_get_cvt_flt_size_fp16(
            kernel_h,
            kernel_w,
            channels);
        
        cvt_filter_ = (__fp16 *)allocator_->Alloc(cvt_filter_size_);
        ppl3_conv_dw_cvt_flt_fp16(
            filter,
            cvt_filter_,
            kernel_h,
            kernel_w,
