...
uint64_t conv2d_n8cx_tile_gemm_cto8c_fp16_runtime_executor::cal_temp_buffer_size() {
    size_t temp_buffer_size = ppl3_tile_gemm_get_temp_buffer_size_riscv_xcto8c_fp16<1>(
...
void conv2d_n8cx_tile_gemm_cto8c_fp16_runtime_executor::adjust_tunning_param() {
    ...
    // int64_t m_factor = (144 / (tunning_param_.oh_blk * tunning_param_.ow_blk)) * (128 / tunning_param_.k_blk);
    // tunning_param_.m_blk = round_up(max((int64_t)1, m_factor), 8);
    // tunning_param_.m_blk = min(tunning_param_.m_blk, round_up(conv_param_->num_output, 8));
}
...
static int64_t ppl3_get_real_filter_size(
    const int64_t flt
) {
    return flt;
}
...
ppl::common::RetCode conv2d_n8cx_tile_gemm_cto8c_fp16_runtime_executor::execute() {
    ...
    ppl3_conv_shell_riscv_fp16<
        ppl3_conv_tile_gemm_tunning_info,
        1,
        ppl3_get_real_filter_size,
        ppl3_conv_tile_gemm_riscv_xcto8c_per_group_fp16<1>>
    (
        src_,
        cvt_filter_,
        cvt_bias_,
...
ppl::common::RetCode conv2d_n8cx_tile_gemm_cto8c_fp16_offline_manager::pick_best_tunning_param(const __fp16 *src, const __fp16 *filter, __fp16 *dst,
    ...
    LOG(INFO) << "tile gemm cto8c best tunning " << best_tunnig_param.m_blk << " " << best_tunnig_param.oh_blk << " " << best_tunnig_param.ow_blk;
    return ppl::common::RC_SUCCESS;
}

ppl::common::RetCode conv2d_n8cx_tile_gemm_cto8c_fp16_offline_manager::gen_cvt_weights(const __fp16 *filter, const __fp16 *bias) {
    ...
    {
        cvt_filter_size_ = ppl3_conv_tile_gemm_get_cvt_filter_size_riscv_xcto8c_fp16<1>(
            kernel_h,
            kernel_w,
            channels,
            num_output,
        ...
        ppl3_conv_tile_gemm_cvt_filter_riscv_xcto8c_fp16<1>(
            filter,
            kernel_h,
            kernel_w,
            num_output,
