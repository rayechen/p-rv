...
static int64_t conv2d_tile_gemm_get_real_filter_size(const int64_t flt) { return flt; }
...
ppl::common::RetCode conv2d_ndarray_tile_gemm_fp32_runtime_executor::execute() {
    ...
    conv2d_shell_fp32<
        conv2d_nxcx_conv_tile_gemm_tunning_info,
        1,
        conv2d_tile_gemm_get_real_filter_size,
...
ppl::common::RetCode conv2d_ndarray_tile_gemm_fp32_offline_manager::pick_best_tunning_param(const float *src, const float *filter, float *dst,
    ...
    for (tunning_param_.m_blk = atom_c; tunning_param_.m_blk <= round_up(num_outs_per_group, atom_c); tunning_param_.m_blk *= 2) {
...
ppl::common::RetCode conv2d_ndarray_tile_gemm_fp32_offline_manager::gen_cvt_weights(const float *filter, const float *bias) {
