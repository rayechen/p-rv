...
uint64_t conv2d_n8cx_wg_b4f3_fp16_runtime_executor::cal_temp_buffer_size() {
    // LOG(INFO) << "n8cx wg b4f3: cal temp buffer size";

    size_t temp_buffer_size = ppl3_conv_wg_bxfxs1_get_temp_buffer_size_fp16<4, 3>(
...
ppl::common::RetCode conv2d_n8cx_wg_b4f3_fp16_offline_manager::pick_best_tunning_param(const __fp16 *src, const __fp16 *filter, __fp16 *dst,
    ...
    for (tunning_param_.oc_blk = 8; tunning_param_.oc_blk <= round_up(param_.num_output / param_.group, 8); tunning_param_.oc_blk *= 2) {
        for (tunning_param_.ic_blk = 8; tunning_param_.ic_blk <= round_up(param_.channels / param_.group, 8); tunning_param_.ic_blk *= 2) {
            for (tunning_param_.oh_blk = 16; tunning_param_.oh_blk <= dst_shape.GetDim(2); tunning_param_.oh_blk += 16) {
                double inner_prev_time = DBL_MAX;
                for (tunning_param_.ow_blk = 16; tunning_param_.ow_blk <= dst_shape.GetDim(3); tunning_param_.ow_blk += 16) {
    ...
    LOG(INFO) << "winograd b4f3 best tunning " << best_tunnig_param.oc_blk << " " << best_tunnig_param.ic_blk << " " << best_tunnig_param.oh_blk << " " << best_tunnig_param.ow_blk;
    return ppl::common::RC_SUCCESS;
}

ppl::common::RetCode conv2d_n8cx_wg_b4f3_fp16_offline_manager::gen_cvt_weights(const __fp16 *filter, const __fp16 *bias) {
    if (cvt_filter_ != nullptr || cvt_bias_ != nullptr) {
        return ppl::common::RC_PERMISSION_DENIED;
    }
    // LOG(INFO) << "n8cx wg b4f3: gen cvt weight";
...

    {
        ...
        memset(cvt_bias_ + num_output, 0.0f, cvt_bias_size_ - num_output);
    }
    {
        cvt_filter_size_ = ppl3_conv_wg_bxfxs1_get_cvt_filter_size_fp16<4, 3>(channels, num_output, group);
        ...
        ppl3_conv_wg_bxfxs1_cvt_filter_fp16<4, 3, wb_b4f3s1_cvt_filter_blk_kernel>(
            filter, // filter,
