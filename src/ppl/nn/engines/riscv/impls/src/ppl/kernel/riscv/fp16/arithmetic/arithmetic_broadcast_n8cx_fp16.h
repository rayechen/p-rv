...
static ppl::common::RetCode arithmetic_broadcast_recursive_n8cx_fp16(
    ...
    if (dim_idx == dim_count - 1) {
        if (src0_shape[dim_idx] == src1_shape[dim_idx]) {
            arithmetic_broadcast_lastdim_no_broadcast_n8cx_fp16<op, fuse_relu>(
                src0, src1, dst, start, end, c0_broadcast, c1_broadcast);
        } else if (src0_shape[dim_idx] == 1) {
            arithmetic_broadcast_lastdim_src0_broadcast_n8cx_fp16<op, fuse_relu>(
                src0, src1, dst, start, end, c0_broadcast, c1_broadcast);
        } else if (src1_shape[dim_idx] == 1) {
            arithmetic_broadcast_lastdim_src1_broadcast_n8cx_fp16<op, fuse_relu>(
                src0, src1, dst, start, end, c0_broadcast, c1_broadcast);
        }
    } else {
