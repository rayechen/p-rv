...
static size_t ppl3_conv_naive_cto8c_get_cvt_filter_size_fp32(
    int64_t flt_h,
    int64_t flt_w,
    int64_t channels,
    int64_t group,
...
static void ppl3_conv_naive_cto8c_cvt_filter_fp32(
    const float *filter,
    int64_t flt_h,
    int64_t flt_w,
    int64_t num_outs,
...
static size_t ppl3_conv_naive_cto8c_get_temp_buffer_size_fp32(
    int64_t src_h,
    int64_t src_w,
    int64_t padding_h,
    int64_t padding_w,
...
static void ppl3_conv_naive_padding_src_cvt_fp32 (
    int64_t src_h,
    int64_t src_w,
    int64_t pad_h,
    int64_t pad_w,
...
static void ppl3_conv_naive_kernel_riscv_fp32(
    const float *src,
    int64_t src_h,
    int64_t src_w,
    int64_t dst_h,
...
)
{
                ...
                for (int64_t dst_w_loc = 0; dst_w_loc < dst_w; ++dst_w_loc) {
                    for (int64_t flt_h_loc = 0; flt_h_loc < flt_h; ++flt_h_loc) {
                        for (int64_t flt_w_loc = 0; flt_w_loc < flt_w; ++flt_w_loc) {
                            ...
                            // LOG(INFO) << dst_per_out[dst_h_loc * dst_w + dst_w_loc] << " " << src_per_channel[src_h_loc * src_w + src_w_loc] << " " << filter[flt_h_loc * flt_w + flt_w_loc];
                        }
                    }
                }
            }
    ...
    for (int i = 0; i < num_outs; i += 1) {
        for (int h = 0; h < dst_h; h += 1) {
            for (int w = 0; w < dst_w; w += 1) {
                LOG(INFO) << i * dst_h * dst_w + h * dst_w + w << " " << dst[i * dst_h * dst_w + h * dst_w + w];
            }
        }
    }
}

static void ppl3_conv_naive_cto8c_general_riscv_fp32(
    const float *src,
    int64_t src_h,
    int64_t src_w,
    int64_t padding_h,
    ...
    bool reflect_pad)
{
    ...
    for (int64_t b = 0; b < batch; b++) {
        ...
        for (int64_t g = 0; g < group; g++) {
            ...
            ppl3_conv_naive_padding_src_cvt_fp32(src_h, src_w, padding_h, padding_w, channels_per_group, src_per_group, temp_src);
            ppl3_conv_naive_kernel_riscv_fp32(
                temp_src,
                src_pad_h,
                src_pad_w,
                dst_h,
...
uint64_t conv2d_ndarray_naive_fp32_runtime_executor::cal_temp_buffer_size() {
    LOG(INFO) << "ndarray naive conv: cal temp buffer size";

    size_t temp_buffer_size = ppl3_conv_naive_cto8c_get_temp_buffer_size_fp32(
...
ppl::common::RetCode conv2d_ndarray_naive_fp32_runtime_executor::prepare() {
    ...
    LOG(INFO) << "ndarray naive conv: prepare";
...
ppl::common::RetCode conv2d_ndarray_naive_fp32_offline_manager::pick_best_tunning_param(const float *src, const float *filter, float *dst,
                                                                                        ppl::nn::TensorShape &src_shape, ppl::nn::TensorShape &dst_shape) {
    return ppl::common::RC_SUCCESS;
}

ppl::common::RetCode conv2d_ndarray_naive_fp32_offline_manager::gen_cvt_weights(const float *filter, const float *bias) {
    if (cvt_bias_ != nullptr || cvt_filter_ != nullptr) {
        return ppl::common::RC_PERMISSION_DENIED;
    }
LOG(INFO) << "ndarray naive conv: gen cvt weights";
    ...
    {
        cvt_filter_size_ = ppl3_conv_naive_cto8c_get_cvt_filter_size_fp32(
            kernel_h,
            kernel_w,
            channels,
            num_group,
        ...
        ppl3_conv_naive_cto8c_cvt_filter_fp32(
            filter,
            kernel_h,
            kernel_w,
            num_output,
