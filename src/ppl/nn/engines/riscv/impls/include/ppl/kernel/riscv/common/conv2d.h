...
public:
    ...
    static const conv2d_common_algo_t direct          = 6;
...
public:
    ...
    void set_conv_param(const conv2d_common_param *conv_param) { conv_param_ = conv_param; }
    const conv2d_common_param *conv_param() const { return conv_param_; };

    void set_cvt_filter(const T *cvt_filter) { cvt_filter_ = cvt_filter; }
    const T *cvt_filter() const { return cvt_filter_; }

    void set_cvt_bias(const T *cvt_bias) { cvt_bias_ = cvt_bias; }
    const T *cvt_bias() const { return cvt_bias_; }

    void set_src(const T *src) { src_ = src; }
    const T *src() const { return src_; }

    void set_src_shape(const ppl::nn::TensorShape *src_shape) { src_shape_ = src_shape; }
    const ppl::nn::TensorShape *src_shape() const { return src_shape_; };

    void set_dst(T *dst) { dst_ = dst; }
    T *dst() const { return dst_; }

    void set_dst_shape(const ppl::nn::TensorShape *dst_shape) { dst_shape_ = dst_shape; }
    const ppl::nn::TensorShape *dst_shape() const { return dst_shape_; }

    void set_sum_src(const T *sum_src) { sum_src_ = sum_src; }
    const T *sum_src() const { return sum_src_; }

    void set_sum_src_shape(const ppl::nn::TensorShape *sum_src_shape) { sum_src_shape_ = sum_src_shape; }
    const ppl::nn::TensorShape *sum_src_shape() const { return sum_src_shape_; }

    void set_temp_buffer(void *temp_buffer) override { temp_buffer_ = temp_buffer; }
    void *temp_buffer() const { return temp_buffer_; }
};
...
public:
    ...
    conv2d_common_algo_info &algo_info() { return algo_info_; }
    void set_algo_info(conv2d_common_algo_info &algo) { algo_info_ = algo; }
...
public:
    ...
    void set_param(const conv2d_common_param &param) { param_ = param; }
    const conv2d_common_param &param() const { return param_; }
    void set_allocator(ppl::common::Allocator *allocator) { allocator_ = allocator; }
    ppl::common::Allocator *allocator() { return allocator_; }
    ...
    const T *cvt_filter() const { return cvt_filter_; }
    uint64_t cvt_filter_size() const { return cvt_filter_size_; }
    ...
    const T *cvt_bias() const { return cvt_bias_; }
    uint64_t cvt_bias_size() const { return cvt_bias_size_; }
...
protected:
    double profile_tunning_param(const T *src, const T *filter, T *dst,
        ...
        conv2d_runtime_executor<T> *executor = dynamic_cast<conv2d_runtime_executor<T>*>(offline_manager.gen_executor());
        auto start = std::chrono::high_resolution_clock::now();
        {
            ...
            if (ppl::common::RC_SUCCESS != (rc = executor->prepare())) {
                LOG(ERROR) << "Prepare failed while the offline manager is picking the best tunning param: " << ppl::common::GetRetCodeStr(rc);
            }
            ...
            for (int32_t i = 0; i < exe_count; i ++) {
                if (ppl::common::RC_SUCCESS != (rc = executor->execute())) {
                    LOG(ERROR) << "Execute failed while the offline manager is picking the best tunning param: " << ppl::common::GetRetCodeStr(rc);
                    return rc;
                }
            }
        }
