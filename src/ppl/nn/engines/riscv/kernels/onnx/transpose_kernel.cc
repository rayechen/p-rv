...
ppl::common::RetCode TransposeKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (data_type == ppl::common::DATATYPE_FLOAT16) {
        return kernel::riscv::transpose_ndarray_fp16(
            data->GetBufferPtr<const __fp16>(),
            transposed->GetBufferPtr<__fp16>(),
            modified_perm.data(),
            &data->GetShape(),
            &transposed->GetShape());
    } else if (data_type == ppl::common::DATATYPE_FLOAT32) {
        return kernel::riscv::transpose_ndarray_fp32(
            data->GetBufferPtr<const float>(),
            transposed->GetBufferPtr<float>(),
            modified_perm.data(),
            &data->GetShape(),
            &transposed->GetShape());
