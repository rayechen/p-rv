...
ppl::common::RetCode AddKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if (output_data_type == common::DATATYPE_FLOAT16) {
        return kernel::riscv::add_fp16(
            &A->GetShape(),
            &B->GetShape(),
            &C->GetShape(),
            false,
            A->GetBufferPtr<const __fp16>(),
            B->GetBufferPtr<const __fp16>(),
            C->GetBufferPtr<__fp16>()
        );
    } else if (output_data_type == common::DATATYPE_FLOAT32) {
        return kernel::riscv::add_fp32(
            &A->GetShape(),
            &B->GetShape(),
            &C->GetShape(),
            false,
            A->GetBufferPtr<const float>(),
            B->GetBufferPtr<const float>(),
            C->GetBufferPtr<float>()            
        );
    } else if (output_data_type == common::DATATYPE_INT64) {
        return kernel::riscv::add_int64(
            &A->GetShape(),
            &B->GetShape(),
            &C->GetShape(),
            false,
            A->GetBufferPtr<const int64_t>(),
            B->GetBufferPtr<const int64_t>(),
