...
ppl::common::RetCode ConcatKernel::DoExecute(KernelExecContext* ctx) {
    ...
    if ((ppl::common::GetSizeOfDataType(data_type) == 4 && real_axis == 1 && data_format == ppl::common::DATAFORMAT_N4CX) ||
        (ppl::common::GetSizeOfDataType(data_type) == 2 && real_axis == 1 && data_format == ppl::common::DATAFORMAT_N8CX)) {
        ...
        if (interleave_channels) {
            if (data_format == ppl::common::DATAFORMAT_N8CX) {
                return kernel::riscv::concat_n8cx_interleave_channels_fp16(
                    (const __fp16**)src_list_.data(),
                    concat_result->GetBufferPtr<__fp16>(),

                    src_shape_list_.data(),
                    ctx->GetInputCount(),
                    real_axis,
                    1);
            } else if (data_format == ppl::common::DATAFORMAT_N4CX) {
                return kernel::riscv::concat_n4cx_interleave_channels_fp32(
                    (const float**)src_list_.data(),
    ...
    if (ppl::common::GetSizeOfDataType(data_type) == 2) {
        if (data_format == ppl::common::DATAFORMAT_NDARRAY) {
            return kernel::riscv::concat_ndarray_fp16(
                (const __fp16**)src_list_.data(), concat_result->GetBufferPtr<__fp16>(),
                src_shape_list_.data(), ctx->GetInputCount(), param_->axis);
        } else if (data_format == ppl::common::DATAFORMAT_N8CX) {
            return kernel::riscv::concat_n8cx_fp16(
                (const __fp16**)src_list_.data(), concat_result->GetBufferPtr<__fp16>(),
                src_shape_list_.data(), ctx->GetInputCount(), param_->axis);
        } else {
        ...
        if (data_format == ppl::common::DATAFORMAT_NDARRAY) {
            return kernel::riscv::concat_ndarray_fp32(
                (const float**)src_list_.data(), concat_result->GetBufferPtr<float>(),
                src_shape_list_.data(), ctx->GetInputCount(), param_->axis);
        } else if (data_format == ppl::common::DATAFORMAT_N4CX) {
            return kernel::riscv::concat_n4cx_fp32(
                (const float**)src_list_.data(), concat_result->GetBufferPtr<float>(),
                src_shape_list_.data(), ctx->GetInputCount(), param_->axis);
        } else {
            LOG(ERROR) << "unsupported data format: " << ppl::common::GetDataFormatStr(data_format);
        ...
        if (data_format == ppl::common::DATAFORMAT_NDARRAY) {
            return kernel::riscv::concat_ndarray_int64(
                (const int64_t**)src_list_.data(), concat_result->GetBufferPtr<int64_t>(),
                src_shape_list_.data(), ctx->GetInputCount(), param_->axis);
        } else if (data_format == ppl::common::DATAFORMAT_N2CX) {
            return kernel::riscv::concat_n2cx_int64(
                (const int64_t**)src_list_.data(), concat_result->GetBufferPtr<int64_t>(),
                src_shape_list_.data(), ctx->GetInputCount(), param_->axis);
        } else {
            LOG(ERROR) << "unsupported data format: " << ppl::common::GetDataFormatStr(data_format);
